...
namespace SaltyServer
{
    public class VoiceManager : BaseScript
    {
        ...
        private readonly HashSet<int> _megaphoneActive = new HashSet<int>();
        #endregion
        ...
        private void OnPlayerDisconnected([FromSource] Player player, string reason)
        {
            ...
            _megaphoneActive.Remove(voiceClient.Player.GetServerId());
        ...
        private void OnSetVoiceRange([FromSource] Player player, float voiceRange)
        {
            ...
            int sid = client.Player.GetServerId();

            if (_megaphoneActive.Contains(sid))
            {
                client.VoiceRange = this.Configuration.VoiceRanges.Min();
                return;
            }
        ...
        private void OnIsUsingMegaphone([FromSource] Player player, bool isSending)
        {
            ...
            int sid = voiceClient.Player.GetServerId();
            if (isSending)
                _megaphoneActive.Add(sid);
            else
                _megaphoneActive.Remove(sid);
            ...
            {
                remoteClient.TriggerEvent(
                    Event.SaltyChat_IsUsingMegaphone,
                    voiceClient.Player.Handle,
