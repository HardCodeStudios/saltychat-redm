...
namespace SaltyClient
{
    internal class VoiceManager : BaseScript
    {
        ...
        public VoiceManager()
        {
            ...
            // NEW: Proximity Setter/Lock Exports (usati dal megafono Lua)
            this.Exports.Add("SetVoiceRange", new Action<float>(this.SetVoiceRangeExternal));
            this.Exports.Add("SetVoiceRangeLocked", new Action<bool>(this.SetVoiceRangeLocked));
        ...
        {
            this.PlaySound("offMicClick", false, "radio");
        ...
        internal void SetVoiceRangeLocked(bool locked)
        {
            this._rangeLocked = locked;
            Debug.WriteLine($"[Salty Chat] Range lock = {locked}");
        }

        internal void SetVoiceRangeExternal(float value)
        {
            if (this.Configuration?.VoiceRanges == null || this.Configuration.VoiceRanges.Length == 0)
                return;

            float best = this.Configuration.VoiceRanges[0];
            float bestDiff = Math.Abs(value - best);

            foreach (var r in this.Configuration.VoiceRanges)
            {
                var d = Math.Abs(value - r);
                if (d < bestDiff) { best = r; bestDiff = d; }
            }

            if (Math.Abs(this.VoiceRange - best) > 0.001f)
            {
                this.VoiceRange = best;
                ...
                {
                    Util.SendChatMessage("Salty Chat",
                        this.Configuration.VoiceRangeNotification.Replace("{voicerange}", this.VoiceRange.ToString()));
                }

                Debug.WriteLine($"[Salty Chat] Forced voice range = {this.VoiceRange}");
            }
        }
        #endregion
        ...
        private void OnVoiceRangePressed()
        {
            ...
            {
                Debug.WriteLine("[Salty Chat] Toggle range pressed but range is locked.");
                return;
            }
